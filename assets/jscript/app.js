...
function plotData(importData) 
{
  ...
  // append to graph
  var dataPointsText = chartGroup.selectAll()
    .data(importData)
    .enter()
    .append("text")
    .attr("x", d => xScale(d.smokes))
    //middle anchor is the bottom of text in the center so I add just a few pixles to 
    //the text into the center
    .attr("y", d => yScale(d.age)+6)
  ...
  dataPointsText.on("mouseover", function(data) 
  {
    toolTip.show(data, this);
  })
  ...
  // mouseout
  dataPoints.on("mouseout", function(data) 
  {
    toolTip.hide(data, this);
  });

  dataPointsText.on("mouseout", function(data) 
  {
    toolTip.hide(data, this);
  });
